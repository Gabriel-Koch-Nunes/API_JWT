...
class AuthIntegrationTests {
    ...
    void setup() {
        userRepository.findByUsername("admin").ifPresentOrElse(
                user -> { /* Já existe */ },
                () -> {
                    com.example.Autenticacao.Autorizacao.model.User admin = new com.example.Autenticacao.Autorizacao.model.User(null, "admin", passwordEncoder.encode("123456"), "ROLE_ADMIN");
        ...
        userRepository.findByUsername("user").ifPresentOrElse(
                user -> { /* Já existe */ },
                () -> {
                    com.example.Autenticacao.Autorizacao.model.User regularUser = new com.example.Autenticacao.Autorizacao.model.User(null, "user", passwordEncoder.encode("password"), "ROLE_USER");
...
void testRegisterUserAlreadyExists() throws Exception {
    mockMvc.perform(post("/auth/register")
            .param("username", "admin")  // Já existente
            .param("password", "qualquer")
            .param("role", "USER")
            .contentType(MediaType.APPLICATION_FORM_URLENCODED))
           .andExpect(status().isBadRequest())
           .andExpect(content().string(containsString("Usuário já existe")));
}
